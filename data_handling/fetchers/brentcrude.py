...
BRENT_SOURCE = "https://oilprice.com/oil-price-charts/#prices"


def fetch_brent_crude_price(rates):
    try:
        # ----------------------------
        # 1. Fetch Page
        # ----------------------------
        ...
        resp = requests.get(BRENT_SOURCE, headers=headers, timeout=15)
        ...
        for row in rows:
            if "Brent Crude" in row.get_text():
        ...
        # ----------------------------
        # 3. Convert USD → INR (using shared FX)
        # ----------------------------
